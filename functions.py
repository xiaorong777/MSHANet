...
from models.EISATC_Fusion import EISATC_Fusion
from models.PENet1 import PENet1
from models.PENet2 import PENet2
...
def train_with_cross_validate(model_name, subject, frist_epochs, eary_stop_epoch, second_epochs, kfolds, batch_size, 
    ...
    dataset_size = len(X_test_tensor)
    indices = np.random.permutation(dataset_size)  
    half_size = dataset_size
    selected_indices = indices[:int(half_size)]  
    X_test_half = X_test_tensor[selected_indices]
    Y_test_half = Y_test_tensor[selected_indices]
...
#%%
def train_with_cross(model_name, subject, frist_epochs, eary_stop_epoch, second_epochs, kfolds, batch_size, 
...
def getModel(model_name, device='cpu'):
    # Select the model
    if(model_name == 'EISATC_Fusion'):
        model = EISATC_Fusion(eeg_chans=22, n_classes=4)
    elif(model_name == 'ATCNet'):
        # Train using the proposed model (ATCNet): https://doi.org/10.1109/TII.2022.3197419
        model = ATCNet(eeg_chans=22, n_classes=4, device=device)   
    elif(model_name == 'TCNet_Fusion'):
        # Train using TCNet_Fusion: https://doi.org/10.1016/j.bspc.2021.102826
        model = TCNet_Fusion(Chans=22, n_classes=4)
    elif(model_name == 'EEGNet'):
        # Train using EEGNet: https://arxiv.org/abs/1611.08024
        model = EEGNet(eeg_chans=22, n_classes=4)
        ...
        model = Deep_ConvNet(kernel_size_spat=3, n_classes=2)
    ...
    elif(model_name == 'PENet1'):
        # Train using EEGNet: https://arxiv.org/abs/1611.08024
        model = PENet1(eeg_chans=22, n_classes=4)
    elif(model_name == 'PENet2'):
        # Train using EEGNet: https://arxiv.org/abs/1611.08024
        model = PENet2(eeg_chans=22, n_classes=4)
    elif(model_name == 'GCNet'):
        # Train using EEGNet: https://arxiv.org/abs/1611.08024
        model = GCNet(samples=1000)
    else:
